...
<script language="javascript"> 
			...
			if (current_count==""){
				...
				remain_count =0;
                        ...
                        if(current_count < AddCount){
                            alert("Invalid Return Stock Amount");
                            return false;
                        }
                         ajremain = parseInt(parseInt(remain_count) + parseInt(AddCount));
                        
			if (who_drug_count){
				who_drug_count = parseInt(parseInt(current_count)- parseInt(who_drug_count));
                                //remain_count = parseInt(parseInt(+who_remain_count)+parseInt(+remain_count));
                                hdrug = parseInt (hdrug + parseInt(who_drug_count));
				var request = $.ajax({
			...
			//var current_count = $("#chk_"+drug_count_id).html();
                        var flage = 0;
                        if (document.getElementById("chk_"+drug_count_id).checked){
                            flage = 1;
                        }    
				...
				var request = $.ajax({
					...
					type: "post",
					data:{"drug_count_id":drug_count_id,"flage":flage}
				});
				 request.done(function (response, textStatus, jqXHR){
					if(response == drug_count_id){
						$("#cell_"+response).html(who_drug_count).removeClass("label-warning").addClass("label-success");
		  ...
		  <div class="col-md-10 ">
				...
				<div class="panel panel-default"  >
					...
					<div class="well well-sm">
						<?php
							if ( empty($drug_stock_list)) {
								...
								echo '<b>Available stocks</b>';
                                                               if($this->session->userdata['UserGroup']=='Programmer' || $this->session->userdata['UserGroup']=='CPharm') 
                                                                   echo '<a class="pull-right btn-xs btn-warning" href="'.site_url("form/create/drug_stock?CONTINUE=drug_stock/view").'">Add new</a>';
                                                               if(isset($drug_stock_info["drug_stock_id"])){    
                                                               echo '<a class="pull-right btn-xs btn-warning" href="'.site_url("form/edit/drug_stock/".$drug_stock_info["drug_stock_id"]."?CONTINUE=drug_stock/view").'">Edit</a>';
                                                               }    
								if ($this->config->item('purpose') !="PP"){
									echo '<br><br> Please select one of the below stock to see the drugs inventory detail.';
								}else{
									echo '<br><br> Drugs inventory detail.';
								}
								echo '<div>';
								...
								for ($i =0; $i<count($drug_stock_list); ++$i){ 
									...
									echo ' "';
                                                                        ...
                                                                        if($drug_stock_list[$i]["UID"] != $this->session->userdata("UID")){                                                                            
                                                                            echo " href='javascript: void(0)'>".$drug_stock_list[$i]["name"]."</a>&nbsp;";
                                                                        }else{
                                                                            
                                                                            echo " href=".site_url("drug_stock/view/".$drug_stock_list[$i]["drug_stock_id"]).">".$drug_stock_list[$i]["name"]."</a>&nbsp;";
                                                                        }
                                                                        }
									//echo " href='{$URL}'>".$drug_stock_list[$i]["name"]."</a>&nbsp;";
                                                                        //echo ' href="">'.$drug_stock_list[$i]["name"].'</a>&nbsp;';
								}       
								echo '</div>';
                      ...
                      <div class="panel panel-default"  > 
                                    <input  id="hdrug" type="hidden" value='<?php echo $drug_count_list[$dstock_id]["drug_stock_id"]; ?>' >
						...
						if (!empty($drug_count_list)){
							...
							echo '<table class="table table-condensed table-bordered table-striped table-hover">';
								echo '<tr >';
									...
									//echo 'Option';
									//echo '</th>';
                                                                     /*   if($drug_count_list[$dstock_id]["drug_stock_id"] != "1"){
                                                                        echo '<th style="font-weight:bold">';
									echo 'Return Stock';
									echo '</th>';
                                                                        } */
								...
								//$dataNumRows = isset($drug_count_list['counts']) ? $drug_count_list['counts']:0;
                                                                echo '<tbody id="fbody">';
                                                               //return;
                                                                //die(print_r($drug_count_list));
                                                                
								for ($i=0; $i < count($drug_count_list) && $i < count($drug_count_list_remain); $i++){
                                                                  // foreach ($drug_count_list as $key ){ 
                                                                    //if(isset($drug_count_list['counts']) ? $drug_count_list['counts']:0){
                                                                    //if($drug_count_list[$i]["who_drug_count"] == $drug_count_list_remain[$i]["who_drug_count"] ){
									echo '<tr>';
									echo '<td>';
									...
									echo '</td>';
                                                                      ...
                                                                      //  if($drug_count_list[$j]["drug_stock_id"] == "1"){
                                                                        echo '<td >';
									...
									echo '</td>'; 
                                                                        ...
                                                                        echo '<td>';
                                                                            if($drug_count_list[$i]["drug_stock_id"] != "1"){    
                                                                            echo '<input  id="pid_'.$drug_count_list[$i]["drug_count_id"].'" type="hidden" value='.$drug_count_list_remain[$i]['pid']. ' >';
                                                                            }
                                                                    ...
                                                                    /*    if($drug_count_list[$j]["drug_stock_id"] != "1"){                              
                                                                         echo '<td >';
										echo ' <input  id="inp2_'.$drug_count_list[$j]["drug_count_id"].'" type="number" class=""  min=0 step=100 value="" style="width: 100px;" >';
										echo ' <input  id="btn2_'.$drug_count_list[$j]["drug_count_id"].'" type="button" onclick=remove_stock('.$drug_count_list[$j]["drug_count_id"].') class="btn btn-default btn-sm" value="Return" >';
									echo '</td>';
                                                                        } */
									echo '<td >';
									...
									echo '</td>';
										...
										//echo ' <input  id="btn_'.$drug_count_list[$j]["drug_count_id"].'" type="button" onclick=add_stock("'.$drug_count_list[$j]["drug_count_id"].'") class="btn btn-default btn-sm" value="Add" >';
									echo '</td>';
								echo '</tr>';
                                                                //}
                                                }
